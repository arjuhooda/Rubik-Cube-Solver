...
for face_color, axis in faces.values():
    for x in (-1, 0, 1):
        for y in (-1, 0, 1):
            ...
            sticker = box(color=face_color, pos=(x, y, 1.5), length=0.98, height=0.98, width=0.05)
...
def mixing(noOfMoves=25, show=1):
    ...
    for i in range(noOfMoves):
        ...
        move(moveList[r], 0)
        if show == 1:
           sys.stdout.write(str(moveList[r]).upper()+ " ")
    if show == 1:
        print("\nCube Mixing !")
...
def solveWhiteCorners(show=1):

    if show == 1:
        print("\nPlacement of white corners :\n")
    global d, u, f, b, r, l, uf, ur, ub, ul, df, dr, db, dl, fr, fl, br, bl, ufr, ufl, ubr, ubl, dfr, dfl, dbr, dbl
    options = ["wrg", "wgo", "wob", "wbr"]
...
def solve2ndLayer(show=1): #Solve the second layer

    if show == 1:
        print("\nConstruction of the second crown:\n")
    global uf, ur, ub, ul, df, dr, db, dl, fr, fl, br, bl
    options = ['rg', 'go', 'ob', 'br'] # Colors of edges by binomial, each letter represents a color, 'rg' means red-green for example

    for i in options: # This line announces a loop, it means "for each element" i "in the list" list 
...
def solveYellowCross(show=1):
    if show == 1:
        print("\nConstruction of the yellow cross :\n")

    global uf, ur, ub, ul

    if uf['u'] != 'y' and ur['u'] != 'y' and ub['u'] != 'y' and ul['u'] != 'y': # If yellow point then :
        move("R'", show)
        move("U'", show)
        move("F'", show)
        move("U", show)
...
def solveFinal(show=1):
    ...
    while 1:  # loop
        ...
        if 'g' in ubr.values() and 'o' in ubr.values(): # If g (green) and o (orange) are contained in the values up-back-right then:
            move("U", show)
            move("U", show)
            move("R", show)
            move("U'", show)
...
solveWhiteCross() # Resolution of the white cross

if isFinish(): # If the cube is solved :
    scene.waitfor('click')
    quit()

solveWhiteCorners() # Resolution of white corners
...
solve2ndLayer() # Second line resolution
...
solveYellowCross() # Resolution of the yellow cross
